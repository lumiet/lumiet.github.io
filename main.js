...
if(alignment==true){
	this.atk = [new attack("Bite", 250, 2, 50, "bite.png", canvas.width-280,canvas.height-75),new attack("Bite", 600, 15, 50, "bite.png", canvas.width-200,canvas.height-75),new attack("Bite", 150, 2, 50, "bite.png", canvas.width-120,canvas.height-75)];
	...
	this.atk = [new attack("Bite", 400, 2, 25, "bite.png", 45,70),new attack("Bite", 20, 2, 15, "bite.png", 125,70),new attack("Bite", 20, 2, 15, "bite.png", 205,70)];
...
function checkatk() {
	...
	if(player.hp>0) enemy.hp-= player.atk[player.currentatk].pwr;
  ...
  if(checkhit(enemyatkx, player, enemy) && enemy.atk[enemy.currentatk].ready) {	  
	  if(enemy.hp>0) player.hp-= enemy.atk[enemy.currentatk].pwr;
...
function setcurratkcolor() {
	for(var i=0;i<player.atk.length;i++) {
		if(player.atk[i].ready) player.atk[i].buttn.color = "#edd976";
		else player.atk[i].buttn.color = "#414141";
	}
	for(var i=0;i<enemy.atk.length;i++) {
		if(enemy.atk[i].ready) enemy.atk[i].buttn.color = "#edd976";
		else enemy.atk[i].buttn.color = "#414141";
	}
}
/////////////////////////////////////////////
function animate() {
